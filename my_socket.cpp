 ...
 *
 * @(#)$Id: my_socket.cpp,v 1.18 2021/02/14 22:01:45 william Exp $
 */
...
void my_getaddrinfo(const string hostname_string, const string port_number_string, struct addrinfo **result)
{
    ...
    for (;;) {
        int rc = getaddrinfo(hostname_string.c_str(), port_number_string.c_str(), &hints, result);
        if (rc == 0) {
            break;
        } else if (rc == EAI_AGAIN) {
            cerr << "getaddrinfo(): " << gai_strerror(rc) << ".  Retry in 1 second..." << endl;
            sleep(1);
            continue;
        }
        cerr << "getaddrinfo() failed: " << gai_strerror(rc) << endl;
        exit(-1);
    }
}
 ...
 * If a client connects, this function would create a new socket file descriptor (which is a copy of the master socket file descriptor) for communicating with the client.
 *
 * You should be able to use this function as it.
 *
